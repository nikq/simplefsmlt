...
Spectrum direct_radiance_sample(const Vec &v0, const Vec &normal, const int id,  const double wavelength, KelemenMLT& mlt) {
  // 光源上の一点をサンプリングする
...
// ray方向からの放射輝度を求める
Spectrum radiance(const Ray &ray, const int depth, const double wavelength, KelemenMLT& mlt ) {
  ...
  case DIFFUSE: {
    if (id != LightID) {
      ...
      for (int i = 0; i < shadow_ray; i ++) {
        direct_light = direct_light + direct_radiance_sample(hitpoint, orienting_normal, id, wavelength, mlt) / shadow_ray;
      }
      ...
      return (direct_light + obj.color * radiance(Ray(hitpoint, dir), depth+1, wavelength,mlt)) / russian_roulette_probability;
    } else if (depth == 0) {
  ...
  case SPECULAR: {
    ...
    return (direct_light + obj.color * radiance(reflection_ray, depth+1, wavelength,mlt)) / russian_roulette_probability;
  } break;
  case REFRACTION: {
    Ray reflection_ray = Ray(hitpoint, ray.dir - normal * 2.0 * Dot(normal, ray.dir));
    ...
    if (depth > MaxDepth) {
      if (mlt.NextSample() < probability) { // 反射
        return
          obj.color * (direct_light + radiance(reflection_ray, depth+1, wavelength,mlt) * Re)
            / probability
              / russian_roulette_probability;
      } else { // 屈折
        return
          obj.color * (direct_light_refraction + radiance(refraction_ray, depth+1, wavelength,mlt)) * Tr
            / (1.0 - probability)
              / russian_roulette_probability;
      }
    } else { // 屈折と反射の両方を追跡
      return
        obj.color * ((direct_light + radiance(reflection_ray, depth+1, wavelength,mlt)) * Re
                     + (direct_light_refraction + radiance(refraction_ray, depth+1, wavelength,mlt)) * Tr) / russian_roulette_probability;
    }
  } break;
  }
...
struct PathSample {
  ...
  double weight;
  PathSample(const int x_ = 0, const int y_ = 0, const double &wavelength_ = 0.f, const double F_ = 0.f, const double weight_ = 1.0) :
  x(x_), y(y_), wavelength(wavelength_), F(F_), weight(weight_) {}
};
double luminance( const PathSample& path ){
  int wavelength_index = (int)(  (path.wavelength * 1000.0) - 360.0 );
  if (wavelength_index >= wl_max) wavelength_index = wl_max - 1;
  return luminance_table[ wavelength_index ] * path.F;
}
Color path2rgb(const PathSample& path ) {
  int wavelength_index = (int)(  (path.wavelength * 1000.0) - 360.0 );
  if (wavelength_index >= wl_max) wavelength_index = wl_max - 1;
  Color xyz( wavelength2xyz_table[ wavelength_index * 3 + 0 ] * path.F,
             wavelength2xyz_table[ wavelength_index * 3 + 1 ] * path.F,
             wavelength2xyz_table[ wavelength_index * 3 + 2 ] * path.F );
  return Color(Dot(Color( 2.3655, -0.8971, -0.4683), xyz),
               Dot(Color(-0.5151,  1.4264,  0.0887), xyz),
               Dot(Color( 0.0052, -0.0144,  1.0089), xyz));
}
...
PathSample generate_new_path(const Ray &camera, const Vec &cx, const Vec &cy, const int width, const int height, KelemenMLT &mlt, int x, int y) {
  ...
  double* p = std::lower_bound(cdf, cdf + wl_max, mlt.NextSample());
...
void render_mlt(Color *image, const Ray &camera, const Vec &cx, const Vec &cy, const int width, const int height) {
  ...
  int SeedPathMax = 65535; //width * height; // 適当に多めの数
  if (SeedPathMax <= 0)
    SeedPathMax = 1;
  ...
  for (int i = 0; i < SeedPathMax; i ++) {
    accumulated_importance += luminance(seed_paths[i]);
    if (accumulated_importance >= rnd) {
      selecetd_path = i;
      break;
    }
  }
  printf("selected_path %d\n",selecetd_path);
